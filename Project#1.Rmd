...
b. Look at residual plots and check for heteroskedasticity (unequal variance), multicollinearity, correlation of errors, and outliers. Transform the data if it is appropriate.  
    ...
    \indent Transformation: Given that the independent variables in this case were all 0 or 1, it was found that most transformations would yield little to no result. However, a log transform of the response variable will result in a slightly better model. The new model is shown below:  
    ```{r}
...
$$log(BoxOfficeGross) = 17.7376 - 0.4223(Comedy) - 1.1104(Documentary) - 0.8001(Drama) - 0.3282(Thriller) - 0.4452(MPAAR)$$  
...
```
Overall, this model is more adequate than the previous one, but that doesn't say much given a very small $R^2$ value, which was 0.1259. Though residuals are now normally distributed, not much of a difference is made in terms of model adequacy.
...
```{r}
Movies2 = Movies[-8,] # We must omit the western movie because it is the only oberservation for that genre.
